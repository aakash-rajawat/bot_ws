...
def main():
    ...
    k_1, k_2, p_1, p_2, k_3, k_4, k_5, k_6 = sf.symbols("k1 k2 p1 p2 k3 k4 k5 k6")
    ...
    x_d = x*((1 + k_1*r2 + k_2*r2**2 + k_3*r2**3)/(1 + k_4*r2 + k_5*r2**2 + k_6*r2**3)) \
          + 2*p_1*x*y + p_2*(r2 + 2*x**2)

    y_d = y*((1 + k_1*r2 + k_2*r2**2 + k_3*r2**3)/(1 + k_4*r2 + k_5*r2**2 + k_6*r2**3)) \
          + 2*p_2*x*y + p_1*(r2 + 2*y**2)
    ...
    inputs = Values(
        ...
        distortion=sf.Matrix([k_1, k_2, p_1, p_2, k_3, k_4, k_5, k_6]),
        sigma_d=Sigma_d
    )
    ...
    codegen = Codegen(
        name="rational_polynomial_undistorted_covariance",
        inputs=inputs,
        outputs=outputs,
        config=CppConfig(),
    )
