...
https://github.com/user-attachments/assets/472537f4-2cb7-4c5c-ab10-b15b3dfc100e
...
`bot_controller` contains the differential-drive motion stack:

- A ROS 2 `diff_drive_controller` configuration for wheel commands, velocity limits, odometry, and TF.
- Joystick teleoperation and `twist_mux` routing for joystick, keyboard, and navigation commands.
- `twist_relay`, which adapts stamped and unstamped velocity commands and publishes a zero command when controller input times out.
- `ua_wheel_odom`, which computes uncertainty-aware wheel-odometry increments for pose-graph fusion.

### Uncertainty-aware wheel odometry

`ua_wheel_odom` adapts the differential-drive kinematic and parametric uncertainty model presented by Carvalho Filho et al. (2019), *The Impact of Parametric Uncertainties on Mobile Robots Velocities and Pose Estimation*. The symbolic adaptation and code-generation steps are documented in [`notebooks/ua_diff_drive_model.ipynb`](notebooks/ua_diff_drive_model.ipynb).

The node reads the angular positions of `base_lw_joint` and `base_rw_joint` from `/joint_states`. For consecutive samples, define the wheel increments as

$$
\Delta\phi_L = \phi_L^k-\phi_L^{k-1},
\qquad
\Delta\phi_R = \phi_R^k-\phi_R^{k-1}.
$$

Using left and right wheel radii $r_L,r_R$, full wheel separation $b$, and lateral center-of-mass offset $y_c$, the relative motion is

$$
\Delta\psi =
\frac{r_R\Delta\phi_R-r_L\Delta\phi_L}{b},
$$

$$
\Delta s =
\frac{r_R\Delta\phi_R+r_L\Delta\phi_L}{2}
+y_c\Delta\psi.
$$

Midpoint integration converts this motion into the planar relative-pose measurement

$$
\delta =
\begin{bmatrix}
\Delta s\cos(\Delta\psi/2) \\
\Delta s\sin(\Delta\psi/2) \\
\Delta\psi
\end{bmatrix}.
$$

This formulation corresponds to the paper's notation with $b=2l$ and $y_c=\Delta P_Y$. Unlike a velocity-only implementation, the node works directly with encoder-position differences; therefore, the sample period cancels from the relative increment.

### Covariance propagation

The node propagates uncertainty in wheel geometry, center-of-mass position, and encoder readings into a full planar covariance:

$$
\Sigma_\delta = J_\delta\,\Sigma_u\,J_\delta^\mathsf{T},
\qquad
J_\delta = \frac{\partial\delta}{\partial u},
$$
...
u =
\begin{bmatrix}
y_c & b & r_L & r_R & \Delta\phi_L & \Delta\phi_R
\end{bmatrix}^{\mathsf{T}}.
$$

The configured standard deviations define a diagonal input covariance. Each encoder increment receives variance $2\sigma_\phi^2$, assuming the two position readings have independent errors.

SymPy derives and simplifies the covariance expressions in the notebook. The generated C++ is split into:

- [`wheel_odom_sigma_static_precomputes.inl`](src/bot_controller/src/generated/wheel_odom_sigma_static_precomputes.inl), evaluated once for calibration-dependent terms.
- [`wheel_odom_sigma_dynamic_precomputes.inl`](src/bot_controller/src/generated/wheel_odom_sigma_dynamic_precomputes.inl), evaluated for each encoder interval.

A small configurable diagonal regularization is added to the resulting $3\times3$ covariance. The node verifies that it is finite and positive semidefinite before publishing the relative measurement.

### ROS interfaces

| Direction | Interface | Purpose |
|---|---|---|
| Subscribes | `/joint_states` | Left and right wheel angular positions |
| Publishes | `/wheel_odometry/pose` | Accumulated planar pose for monitoring |
| Publishes | `/bot_controller/relative_pose_wheel` | Timestamped relative pose and covariance consumed by the GTSAM pose graph |

The relative-pose topic is configurable through `topic_incremental_pose`. Geometry and uncertainty parameters are defined in [`ua_wheel_odom.yaml`](src/bot_controller/config/ua_wheel_odom.yaml).

The covariance is a first-order local approximation under independent input uncertainties. It models encoder noise and uncertainty in wheel radii, wheel separation, and lateral center-of-mass offset. Wheel slip, backlash, terrain interaction, and temporal correlations between successive measurements are not explicitly represented.
